#!/usr/bin/env bash
# sync.sh — smart portable-drive <-> GitHub sync.
#
#     bash .brain/tools/sync.sh
#
# Safe by construction: fast-forward pulls only, never force-pushes, never
# auto-merges, and works offline (it just tells you to carry on).
# Pushes only after you confirm, or with SYNC_PUSH=1.
#
# Origin: written for the task-system rebuild, lifted into the brain unchanged
# in behaviour.
set -euo pipefail
...
require_git() {
  if ! command -v git >/dev/null 2>&1; then
    ...
    say "    Install Git (Xcode CLT or https://git-scm.com), then re-run bash .brain/tools/sync.sh"
    ACTION_LINE="install Git, then re-run bash .brain/tools/sync.sh"
    return 1
  fi
  if ! git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    err "This folder is not a Git repository yet."
    say "    Folder: $ROOT"
    say "    Next: connect this folder to GitHub once, then sync will work."
    say "    One-time setup:"
    say "      cd \"$ROOT\""
    say "      git init"
    say "      git remote add origin <your-github-repo-url>"
    say "      git add -A && git commit -m \"initial commit\""
...
# --- main ---
hdr "$NAME sync (portable drive <-> GitHub)"
say "folder: $ROOT"
...
if [ "$ONLINE" -eq 0 ]; then
  ...
  compute_ahead_behind
  sync_label
  ACTION_LINE="work normally on the portable drive; run bash .brain/tools/sync.sh again when online"
  hdr "Status summary"
  say "  Network : offline"
  say "  Branch  : $BRANCH"
  ...
  say "When you have internet again, run:  bash .brain/tools/sync.sh"
  say ""
  exit 0
fi
...
if ! git fetch "$REMOTE" --prune; then
  ...
  ACTION_LINE="fix GitHub auth, then re-run bash .brain/tools/sync.sh"
  hdr "Status summary"
  say "  Network : online"
  say "  Branch  : $BRANCH"
  say "  Action  : $ACTION_LINE"
...
if [ "${BEHIND:-0}" -gt 0 ] && [ "${AHEAD:-0}" -eq 0 ]; then
  ...
  if working_tree_dirty; then
    ...
    say "      bash .brain/tools/sync.sh"
    ACTION_LINE="commit local changes, then re-run bash .brain/tools/sync.sh to pull"
  else
...
if [ "${AHEAD:-0}" -gt 0 ] && [ "${BEHIND:-0}" -eq 0 ]; then
  ...
  if [ "${SYNC_PUSH:-}" = "1" ]; then
    ...
    warn "Not a terminal — skip push (re-run interactively, or SYNC_PUSH=1 bash .brain/tools/sync.sh)"
    ACTION_LINE="re-run bash .brain/tools/sync.sh and answer y to push (or SYNC_PUSH=1)"
  fi
  ...
  if [ "$DO_PUSH" -eq 1 ]; then
    if git push "$REMOTE" "$BRANCH"; then
      ...
      say "    No force-push was attempted. Fix access and try bash .brain/tools/sync.sh again."
      ACTION_LINE="fix push access, then re-run bash .brain/tools/sync.sh"
    fi
  else
    ACTION_LINE="local commits waiting — run bash .brain/tools/sync.sh and choose y to push"
